...
public class Network  extends Environment {
    ...
    public static final Literal checkWorkLoadRule = Literal.parseLiteral("checkWorkLoadRule");
    ...
    //    textName= turnScope+"_boolean_";
        ...
        MapDataPath = "C:\\Users\\Burak\\eclipse-workspace_earlier\\networkScaler\\src\\java\\inputs\\"+turnScope+"\\Workload_"+turnScope+"_Iteration"+turn+".txt"; // Change this to your file path
         ...
         for (int i = 0; i < values.length; i++) {
    		...
    		removePerceptsByUnif("agentFuzzy",Literal.parseLiteral("currentWorkLoad(_)")); // modified from agent1
    		...
    		removePerceptsByUnif("agentRuleFuzzy",Literal.parseLiteral("currentWorkLoad(_)"));
        	Literal literal_currentWorkLoadRuleFuzzy = Literal.parseLiteral("currentWorkLoad(0)");
    		addPercept("agentRuleFuzzy",literal_currentWorkLoadRuleFuzzy);
   ...
   /*     try {
    ...
    public boolean executeAction(String ag, Structure action) {
        ...
        try {
	     ...
	     //   	 printTheMap("End-Map ",garbageHolder);
            ...
            else if (action.equals(checkWorkLoadRule)) {
            	checkWorkLoadRule(ag);
            }
    	 ...
    	 private void saveResult(Term decisionResult, int planNumber)
	{
		...
		String outputFilePath = "C:\\Users\\Burak\\OneDrive - Universiteit Antwerpen\\Bureaublad\\Activities\\Publications\\TSF\\Case Studies\\Production Line\\OutputFiles\\WCET\\Yeni Olcumler\\NetworkScaler\\PC\\FuzzyIntegrated\\output.txt";

		// Append planNumber and elapsedMillis to file
		try (BufferedWriter writer = new BufferedWriter(new FileWriter(outputFilePath, true))) {
			writer.write("PlanNumber: " + planNumber + ", ElapsedMillis: " + elapsedMillis);
    ...
    public Integer[] getNextValue() {
        ...
        if (currentIndex < values.length) {
            ...
            try (BufferedWriter writer = new BufferedWriter(new FileWriter(filePathTotal))) {
                ...
                System.out.println("File created successfully at 2 " + filePathTotal);
            } catch (IOException e) {
                System.err.println("Error writing to file: " + e.getMessage());
            }
            ...
            try (BufferedWriter writer2 = new BufferedWriter(new FileWriter(fileExecutionTimePathTotal,true))) {
              ...
              //  writer.write(totalMach+" "+workloadTurnCounter);
                System.out.println("File created successfully at 3 " + filePathTotal);
            } catch (IOException e) {
                System.err.println("Error writing to file: " + e.getMessage());
            }
	...
	public void checkWorkLoadRule(String ag)
    {
    ...
    /* 
      ...
      */
    	...
    	removePerceptsByUnif(ag,Literal.parseLiteral("workLoad(low,_)"));
    	Literal literal_workLoadLow = Literal.parseLiteral("workLoad(low,"+workLoadLow+")");
    	addPercept(ag,literal_workLoadLow);
    	
    	
    	double workLoadMid = trape(50.0,150.0,300.0,400.0,workLoad);
    	removePerceptsByUnif(ag,Literal.parseLiteral("workLoad(medium,_)"));
    	Literal literal_workLoadMid = Literal.parseLiteral("workLoad(medium,"+workLoadMid+")");
    	addPercept(ag,literal_workLoadMid);


    	double workLoadMax = trape(250.0,400.0,5000.0,5000.0,workLoad);
    	removePerceptsByUnif(ag,Literal.parseLiteral("workLoad(high,_)"));
    	Literal literal_workLoadMax = Literal.parseLiteral("workLoad(high,"+workLoadMax+")");
    	...
    	removePerceptsByUnif(ag,Literal.parseLiteral("responseTime(good,_)"));
    	Literal literal_responseTimeMin = Literal.parseLiteral("responseTime(good,"+responseTimeMin+")");
    	addPercept(ag,literal_responseTimeMin);
    	
    	
    	double responseTimeMid = trape(5.0,7.5,7.5,10.0,responseTime);
    	removePerceptsByUnif(ag,Literal.parseLiteral("responseTime(ok,_)"));
    	Literal literal_responseTimeMid = Literal.parseLiteral("responseTime(ok,"+responseTimeMid+")");
    	addPercept(ag,literal_responseTimeMid);


    	double responseTimeMax = trape(7.5,10.0,100.0,100.0,responseTime);
    	removePerceptsByUnif(ag,Literal.parseLiteral("responseTime(bad,_)"));
    	Literal literal_responseTimeMax = Literal.parseLiteral("responseTime(bad,"+responseTimeMax+")");
    	addPercept(ag,literal_responseTimeMax);
   ...
   //            + batteryStart + "_" + vacuumStart + "_" + mapNumber + "_OutBoolean.txt";
       ...
       try (BufferedWriter writer = new BufferedWriter(new FileWriter(filePathTotal, true))) {         
           writer.write(toFile);
           System.out.println("File created successfully at 4 " + filePathTotal);
       } catch (IOException e) {
           System.err.println("Error writing to file: " + e.getMessage());
       }
